...
func runService(cnf *config, logger, errorLogger *stdlog.Logger) error {
	...
	err = server.ServiceRegistryWithConsul(appID+"-grpc", cnf.ServiceID, grpcPort, ":"+restPort+"/health", []string{"urlprefix-/api.Counter" + " proto=grpc grpcservername=" + cnf.ServiceID})
	if err != nil {
		errorLogger.Println(err)
		return err
	}
