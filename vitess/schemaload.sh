...
schema_files=${SCHEMA_FILES:-'default_schema.sql'}
...
if [ ! -f schema_run ]; then
  ...
  vtctlclient -server vtctld:$GRPC_PORT ApplyVSchema -vschema_file /script/vschema/${vschema_file} $KEYSPACE || \
  vtctlclient -server vtctld:$GRPC_PORT ApplyVSchema -vschema "$(cat /script/vschema/${vschema_file})" $KEYSPACE
